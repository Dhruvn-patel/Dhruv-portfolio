...
const FloatingNavbar = ({ navItems, className }: { navItems: INavItem[]; className?: string }) => {
  ...
  return (
    <>
      {/* Full-width scroll progress bar at top of screen */}
      <motion.div
        style={{ scaleX: scrollX }}
        className="fixed top-0 left-0 right-0 h-[3px] bg-[var(--primaryColor)] origin-left z-[6000]"
      />
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={cn(
            "flex w-fit fixed top-2 sm:top-4 inset-x-0 mx-auto border border-white/[0.25] rounded-full bg-[var(--dialogColor50)] backdrop-blur-sm shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-3 sm:px-4 py-2 sm:py-3 items-center space-x-2 sm:space-x-4",
            className
          )}
        >
          <Row classNames="w-full justify-center items-center">
            <Row classNames="gap-2 sm:gap-4 items-center">
              {navItems.map((navItem: INavItem, idx: number) => (
                <Link
                  key={`link=${idx}`}
                  href={navItem.link}
                  className={cn("relative flex items-center space-x-1 text-neutral-50 group p-2 sm:p-1")}
                >
                  <span className="block sm:hidden relative overflow-hidden">
                    ...
                    <span className="absolute inset-0 text-[var(--primaryColor)] transition-transform transform translate-y-full group-hover:translate-y-0 duration-300 ease-in-out z-10 text-lg">
